function newVals=dirichletboundary(L,index,vals,superpixels,m,n,sal,spno,mu)
...
%Yu
Y=sal2pixel(superpixels,m,n,sal,spno);
Yu=[Y(antiIndex),Y(antiIndex)];


%Find RHS
b=-L(antiIndex,index)*(vals)+mu*Yu;

%Solve system
x=(L(antiIndex,antiIndex)+mu*sparse(1:length(antiIndex),1:length(antiIndex),1))\b;
